...
const formatDate = (dateString) => {
  const options = { day: "2-digit", month: "short", year: new Date(dateString).getFullYear() !== new Date().getFullYear() ? "numeric" : undefined, hour: 'numeric',
  minute: 'numeric'};
...
const token = JSON.parse(localStorage.getItem("myUser"));
  ...
  const handleEditReply = async (replyId) => {
    const sanitizedEditText = DOMPurify.sanitize(editReplyId.text);
    if (sanitizedEditText.length > 300) {
      return toast.error("Reply length exceeds the limit (300 characters).", {
        icon: "😬",
        id: "reply",
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URI}/comm/editReply/${replyId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      const updatedComments = comments.map((comment) => ({
        ...
        replies: comment.replies.map((reply) =>
          reply._id === replyId ? data : reply
        ),
      }));
      setComments(updatedComments);
      setEditReplyId(null);
    } catch (error) {
      toast.error("Failed to edit reply. Please try again later.", {
        id: "editfail",
      });
      console.error(error);
    }
            ...
            >
              <footer className="flex justify-between items-center mb-2">
                <div className="flex items-center">
                 ...
                 <FaRegUserCircle />
                    <span className="ml-3 tracking-wider text-base text-blue-400 dark:text-blue-200 ">{comment.username}</span>
                  </p>
                ...
                {comment.userId === user.user_id && <div className="relative inline-block">
                <button
                  ...
                  type="button" onClick={(e)=>{handleDropdownClick(comment._id,e)}}
                >
                  <svg
                    className="w-4 h-3"
                    aria-hidden="true"
                ...
                {showDropdown === comment._id && (
                      <div ref={dropdownRef} className="absolute md:left-4 w-14 md:w-20 origin-top-right focus:outline-none bg-blue-100 rounded-md shadow-lg dark:bg-gray-800 z-10">
                        <button className="flex w-full items-center rounded-md hover:bg-blue-200 dark:hover:bg-gray-700" onClick={() => { handleEditClick(comment._id,comment.text) }}>
    <HiPencilSquare className="dark:text-white pl-2 text-base hidden md:block md:text-xl"/>
    <p
      className="ml-2 py-2 text-xs md:text-sm text-gray-800 text-left dark:text-gray-200"
    >
      Edit
    </p>
  </button>
  <button className="flex w-full items-center rounded-md hover:bg-blue-200 dark:hover:bg-gray-700"  onClick={() => { handleDeleteClick(comment._id) }}>
    <IoTrashBin className="dark:text-white pl-2 text-base hidden md:block md:text-xl"/>
    <p
      className="ml-2  py-2 text-xs md:text-sm rounded-md text-left text-gray-800 dark:text-gray-200"
    >
      Delete
    </p>
  </button>
              ...
              {editComment === comment._id ? (
                    ...
                    <p className="text-gray-900 dark:text-white text-wrap" style={{ overflowWrap: 'break-word' }} >{comment.text}</p>
                  )}
                  <div className="flex items-center mt-4 space-x-4">
                    <button
                      type="button"
                      className="flex items-center text-sm text-blue-400 cursor-pointer hover:underline underline-offset-1 dark:text-blue-200 font-medium" onClick={()=>{handleReplySubmit(comment._id)}}
                    >
                      <svg
                ...
                <div key={reply._id} className="ml-8 mt-4">
                  <footer className="flex justify-between items-center mb-2">
                    <div className="flex items-center">
                      <p className="inline-flex items-center mr-3 text-sm text-blue-400 dark:text-blue-200 font-semibold">
                        <FaRegUserCircle />
                        <span className="ml-3 tracking-wider text-base text-blue-400 dark:text-blue-200 ">
                          {reply.username}
                        </span>
                      </p>
                      <p className="text-sm text-gray-500 dark:text-gray-500 ml-2">
                    ...
                    {reply.userId === user.user_id && (
                      <div className="relative flex flex-row space-x-6">
                          ...
                          <HiPencilSquare className="dark:text-white text-sm md:text-base lg:text-lg block cursor-pointer" onClick={()=>{handleEditReply(reply._id)}}/>
                          <IoTrashBin className="dark:text-white text-sm md:text-base lg:text-lg block cursor-pointer" onClick={()=>{handleDeleteReply(reply._id)}}/>
                        
                      </div>
                    )}
                  </footer>
                  {editReplyId && editReplyId._id === reply._id ? (
                    <div className="py-2 px-4 mb-4 bg-white rounded-lg rounded-t-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                      <textarea
                        id="reply"
                        rows="3"
                        value={editReplyId.text}
                        onChange={(e) =>
                          setEditReplyId({ ...editReplyId, text: e.target.value })
                        }
                        className="px-0 w-full text-base mb-2 text-gray-900 border-0 focus:ring-0 focus:outline-none dark:text-white dark:bg-gray-800"
                        ref={(textarea) => {
                          replyInputRef.current = textarea;
                          if (textarea && editReplyId) {
                            textarea.focus();
                    ...
                    <p className="text-gray-900 dark:text-white text-wrap" style={{ overflowWrap: 'break-word' }} >
                      {reply.text}
                    </p>
                  )}
                </div>
